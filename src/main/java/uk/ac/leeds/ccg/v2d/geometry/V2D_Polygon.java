...
/**
 * Comprising two collections of V2D_ConvexHull one representing parts and
...
public class V2D_Polygon extends V2D_FiniteGeometry {
     ...
     */
    protected final ArrayList<V2D_ConvexHull> holes;
     ...
     * @param oom, rm The tolerance within which two vectors are regarded as
     * equal.
     */
    ...
    public V2D_Polygon(V2D_ConvexHull ch, 
            ...
            ArrayList<V2D_ConvexHull> holes) {
        super();
        this.ch = ch;
        this.edges = edges;
        this.holes = holes;
    ...
    public String toString() {
        ...
        }
        {
            if (holes != null) {
                s += "\nHoles(\n";
                Iterator<V2D_ConvexHull> ite = holes.iterator();
                s += ite.next().toString();
                while (ite.hasNext()) {
                    s += ", " + ite.next();
                }
     ...
     * @return A copy of {@link holes} with the given tolerance applied.
     */
    public ArrayList<V2D_ConvexHull> getHoles(int oom, RoundingMode rm) {
        ArrayList<V2D_ConvexHull> r = new ArrayList<>();
        for (V2D_ConvexHull h : holes) {
            r.add(new V2D_ConvexHull(h, oom, rm));
        }
        return r;
    }
        ...
        if (t.isIntersectedBy(getEnvelope(oom, rm), oom, rm)) {
            if (isIntersectedBy(t.getEnvelope(oom, rm), oom, rm)) {
                if (getConvexHull(oom, rm).isIntersectedBy(t, oom, rm)) {
                    for (V2D_ConvexHull h : getHoles(oom, rm)) {
...
//        return sum;
    ...
    public V2D_Polygon rotateN(V2D_Point pt, BigRational theta, 
        ...
        ArrayList<V2D_ConvexHull> rholes = new ArrayList<>();
        ArrayList<V2D_ConvexHull> tholes = getHoles(oom, rm);
        if (tholes != null) {
